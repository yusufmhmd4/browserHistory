...
class History extends Component {
  state = {historyList: initialHistoryList, searchInput: '', noWebsite: false}
  ...
  onDeleteSearch = id => {
    const {historyList, noWebsite} = this.state

    const updatedHistoryList = historyList.filter(each => each.id !== id)
    if (historyList.length === 1) {
      this.setState({noWebsite: true})
    }

    this.setState({historyList: updatedHistoryList})
    console.log(noWebsite)
    console.log(historyList.length)
  }

  render() {
    const {historyList, searchInput, noWebsite} = this.state
    let value = noWebsite
      ...
      <div className="main-container">
        ...
        {!value ? (
          ...
          <div className="no-website-container">
            {' '}
            <p className="no-website">There is no history to show</p>
          </div>
        )}
      </div>
    )
